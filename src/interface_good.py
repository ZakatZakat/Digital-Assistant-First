...
serpapi_key = '3d8b36b03f70a98e022e3bf5c0a08f02d70175cb347c2da7f2cb40603f576365'
...
def model_response_generator(retriever, model, config):
    ...
    if int(config['history_size']) and len(st.session_state["messages"]) > 0:
        logger.info(f"Сообщения в сессии: {st.session_state['messages']}")
    ...
    if config['System_type'] in ['RAG', 'File']:
        ...
        system_prompt = (
            "Ты являешься цифровым помощником сервиса ВТБ Консьерж"
            "Отвечай на вопросы как можно более развернуто. Если ответа на вопрос нет в контексте "
            "— отвечай, что не можешь предоставить информацию. Если история сообщений "
            "предоставлена, то также используй её для ответа на вопрос:\n\n {context}"
        ) + message_history + 'Используй также информацию из интернета c ссылками:' + str(internet_res) +  str(links) + '\n\n И также предложи что-то из объявлений указанных тут' + str(shopping_res) + '\n\n И также используй информацию с google maps представленную ниже' + str(maps_res)
        

        print('sss', system_prompt)
        ...
        if config['Model'].startswith('gpt'):
            ...
            # Добавляем отступ между частями
            ...
            prompt_template = (
                "Представь, что ты менеджер банка ВТБ, который старается дать наиболее точный "
                "ответ на вопрос клиента. Не выдумывай ничего, отвечай только на основании информации "
                "из контекста. Если история сообщений предоставлена, то также используй её для ответа "
                "на вопрос:" + message_history + "\n\nКонтекст: {context}\n\nВопрос: {question}"
            )

            if config['Splitter']['Type'] == 'character':
                context_docs = retriever.similarity_search(query=user_input, k=1)
            elif config['Splitter']['Type'] == 'json':
                context_docs = [
                    doc for doc, _ in retriever.similarity_search_with_score(user_input, k=1)
                ]
            else:
                context_docs = []
        ...
        system_prompt = (
            "Ты являешься цифровым двойником помощником для работы с файлами компании. "
            "Отвечай на вопросы как можно более развернуто. Если история сообщений "
            "предоставлена, то также используй её для ответа на вопрос:" + message_history
        )
        ...
        if config['Model'].startswith('gpt'):
            ...
            chain = prompt | model
            streaming_answer = chain.stream(user_input)
            for token in streaming_answer:
                yield token
        else:
            prompt = ChatPromptTemplate.from_messages(
                [("system", system_prompt), ("human", "{input}")]
            )
            st.session_state["messages"][-1]['content'] = prompt.format(
                input=user_input
            )
            stream = ollama.chat(
                model=model, messages=st.session_state["messages"], stream=True
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            message = st.write_stream(
                model_response_generator(retriever, model, config)
            )
            st.session_state["messages"].append(
                {"role": "assistant", "content": message, 'question': prompt}
            )
