...
def test_selectbox_options():
    ...
    assert set(at.selectbox[0].options).issuperset(['gpt-4o', 'gpt-4o-mini']), 'Опции моделей неправильны'
    
    # Проверяем первый selectbox "Выберите тип системы"
    assert at.selectbox[1].options == ['default', 'RAG', 'File'], "Опции первого selectbox не совпадают"

    # Проверяем второй selectbox "Выберите тип цепочки"
    assert at.selectbox[2].options == ['refine', 'map_reduce', 'stuff'], "Опции второго selectbox не совпадают"

    # Проверяем третий selectbox "Выберите модель эмбеддингов"
    assert at.selectbox[3].options == ['BAAI/bge-m3', 'sentence-transformers/all-MiniLM-L6-v2', 'OpenAIEmbeddings'], "Опции третьего selectbox не совпадают"
...
def test_RAG_function():
    ...
    assistant_messages = [message['content'] for message in result.session_state.messages if message['role'] == 'assistant']
    ...
    assert any(all(word in message for word in words_to_check) for message in assistant_messages), "Не все слова найдены в одном сообщении ассистента"
...
def test_default_function():
    ...
    assert not any(all(word in message for word in words_to_check) for message in assistant_messages), "Некоторое сообщение содержит все слова"
